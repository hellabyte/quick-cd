...
function cd(){
    ...
    if [[ $(($CD_COUNT % 24)) -eq 0 ]]; then
        awk -F/ '{ if( $7 ) { 
            print $2FS$3FS$4FS$5FS$6 
        } else { print } }' $GENERAL_DIRS | sort -u > $TEMP_DIRS 2> /dev/null
    ...
    if [ $(($CD_COUNT % 12)) -eq 0 ]; then
        ...
        GEN_DIRS_UNIQ_COUNT=$(\
            sort $GENERAL_DIRS | uniq -c | wc -l | awk '{print $1}')
        if [ $GEN_DIRS_DIFFERENCE -gt 25 ]; then
            GEN_DIRS_JUNK_LIMIT=".45"; 
            GEN_DIRS_PROP_COUNT=$(\
                python -c "${GEN_DIRS_UNIQ_COUNT}./$GEN_DIRS_LINE_COUNT")
            GEN_DIRS_PY_STRING=\
                "print 0 if $GEN_DIRS_PROP_COUNT <= $GEN_DIRS_JUNK_LIMIT else 1"
